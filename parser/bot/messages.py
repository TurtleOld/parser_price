...
def get_price_history(product):
    return [(entry['updated_at'], entry['price'], entry['price_ozon']) for entry
            in product['prices_history']]
...
async def send_price_graph(chat_id, product_name, price_history, product_id):
    ...
    keyboard = InlineKeyboardMarkup()
    back_button = InlineKeyboardButton(text="Вернуться",
                                       callback_data=f"return_to_card_{product_id}")
...
async def handle_get_prices(message):
    ...
    for result in results:
        for index, product in enumerate(result.get('products', [])):
            ...
            button = InlineKeyboardButton(text=product_name,
                                          callback_data=f"product_{index}")  # Кнопка для каждого товара
            keyboard.add(button)

    if len(keyboard.keyboard) > 0:  # Проверяем, есть ли кнопки
        await bot.send_message(chat_id=user_id, text="Выберите товар:",
...
async def callback_product(call):
    ...
    if result:
        ...
        keyboard = create_product_keyboard(
            product_index)  # Используем индекс как уникальный идентификатор
...
async def callback_return_to_card(call):
    product_id = int(
        call.data.split('_')[3])  # Извлекаем ID товара из callback_data
    user_id = call.message.chat.id
    result = messages.find_one({'telegram_user_id': user_id}, {'_id': 0})
    if result:
...
async def get_url(message):
    ...
    result_insert_data = insert_data(available=available, user_id=user_id,
                                     url=result_parse_url,
                                     product_name=product_name_dict['title'],
                                     price=price, price_ozon=card_price,
